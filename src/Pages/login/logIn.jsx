...
const LogIn = () => {
    ...
    const form = useFormik({
      ...
      .then((res) => {
        const result = lodash.filter(res.data, {"userName": values.name,"password":values.password} );
    ...
    return ( 
        <div className="text-center px-4 py-4">
              <form onSubmit={form.handleSubmit} className="col-lg-6 col-md-5 col-10 mx-auto cart-style p-4 px-5 clearfix">
                <LogoImg/>
                <div className="d-flex flex-column cart-body gap-2 text-center mt-5">
                  <div className="form-floating text-muted">
                    ...
                    {form.errors.name && form.touched.name && <div className='text-danger'>{form.errors.name}</div>}
                  </div>
                  <div className="form-floating text-muted">
                    <input type="password"
                           name="password" 
                    ...
                    {form.errors.password && form.touched.password && <div className='text-danger'>{form.errors.password}</div>}
                  </div>
                  <ReCAPTCHA
                  ...
                  <Link to='/main' className='text-decoration-none mx-2 dark'>Forget Password?</Link>
                </div>
              </form>
              ...
              <footer className='col-lg-6 col-md-5 col-5 d-md-block d-none mx-auto pt-4 mt-3 p-3 mb-3'>
                  <div className="row row-cols-lg-6 row-cols-md-5 row-cols-3" >
                      <Link to='#' className='text-decoration-none dark'>ABOUT US</Link>
                      <Link to='#' className='text-decoration-none dark'>SUPPORT</Link>
                      <Link to='#' className='text-decoration-none dark'>BLOG</Link>
                      <Link to='#' className='text-decoration-none dark'>PRESS</Link>
                      <Link to='#' className='text-decoration-none dark'>API</Link>
                      <Link to='#' className='text-decoration-none dark'>JOBS</Link>
                  </div>
                  <div className="row row-cols-lg-4 row-cols-md-4 row-cols-2 mt-2" >
                      <Link to='#' className='text-decoration-none dark'>PRIVACY</Link>
                      <Link to='#' className='text-decoration-none dark'>TERMS</Link>
                      <Link to='#' className='text-decoration-none dark'>DIRECTORY</Link>
                      <Link to='#' className='text-decoration-none dark'>LANGUAGE</Link>
                  </div>
              </footer>
        </div>
    );
